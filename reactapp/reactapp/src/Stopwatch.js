  ...
  startTimer = () => {
    this.interval = setInterval(() => {
      this.setState((prevState) => ({
        miliSec: prevState.miliSec === 100 ? 0 : prevState.miliSec + 1,
        Sec: prevState.miliSec === 60 ? prevState.Sec + 1 : prevState.Sec,
      }));

      if (this.state.Sec === 60) {
        this.setState((prevState) => ({
          min: prevState.Sec === 60 ? prevState.min + 1 : prevState.min,
          Sec: 0,
        }));
      }
    }, 1000);
        ...
        <div className="App">
          <div className="App2">
            ...
            {this.state.pause === true ? (
              ...
              <button onClick={this.resumeTimer} style={{ margin: '5px' }} data-testid="resume">
                Resume
              </button>
            ) : (
              <button onClick={this.startTimerNow} style={{ margin: '5px' }} data-testid="start">
                Start
              </button>
            )}

            {this.state.reset === false ? (
              <button data-testid="reset" className="rsbtn" onClick={this.resetTimer} disabled="disabled">
                Reset
              </button>
            ) : (
              <button onClick={this.resetTimer} data-testid="reset" className="rsbtn">
                Reset
              </button>
            )}
